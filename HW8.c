...
struct tree_node* parent(struct tree_node *root, struct tree_node *node) 
{
    // 1. Take care of NULL cases and the case of only one node in which case there wouldn't be any parent 
      if(root == NULL){
          return NULL;
      }
      else if(root == node){
...
struct tree_node* minVal(struct tree_node *root) 
{
    // Check if root stores the minimal value.
    if(root == NULL){
          return NULL;
      }
    ...
    // Else call the minval function recursively with the left subtree
    else{
...
{   
    ...
    if(current_ptr != NULL){
        // Check if the value at the root is the node you are looking for.
...
{
    struct tree_node* temp;
    struct tree_node* keynode;
    struct tree_node* replaceNode = NULL; 
    int someData;
    
    temp = findNode(root, value);
    keynode = parent(root, temp);
    ...
    //Now handle the case where the node to be deleted has 2 children
    else{
    
    //Find the new physical node to delete. (call the minval function)
    replaceNode = minVal(temp -> right);
    someData = replaceNode -> data;
    //Call the delete function with the root and value of the node returned by the minval function
    delete(root, someData);
